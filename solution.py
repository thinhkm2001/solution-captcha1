...
class PuzleSolver:
    def __init__(self, piece: np.ndarray=None, background: np.ndarray=None):
        self.piece = piece
    ...
    def __background_preprocessing(self, y_sup, y_inf):
        background = self.__sobel_operator(self.background)
    ...
    def __piece_preprocessing(self):
        img = self.__sobel_operator(self.piece)
    ...
    def __extend_template_boundary(self, template):
        extra_border = np.zeros(
            (template.shape[0], PIXELS_EXTENSION), dtype=int)
        template = np.hstack((extra_border, template, extra_border))

        extra_border = np.zeros(
            (PIXELS_EXTENSION, template.shape[1]), dtype=int)
    ...
    def __extend_background_boundary(self, background):
        extra_border = np.zeros(
            (PIXELS_EXTENSION, background.shape[1]), dtype=int)
        return np.vstack((extra_border, background, extra_border))

    def __sobel_operator(self, img: np.ndarray=None):
        scale = 1
        delta = 0
        ddepth = cv2.CV_16S
